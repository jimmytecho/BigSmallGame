...
batch_size = 5
EPISODES = 50000
counts = np.zeros((int(EPISODES/100), 100))
wincounts = np.zeros((int(EPISODES/100), 100))
...
for e in range(EPISODES):
	...
	for time in range(5):
		...
		if len(agent.memory) >= batch_size:
			agent.replay()
	counts[int(e/100)][e%100] = rounds
	if points1>=0 and win1 > win2:
		wincounts[int(e/100)][e%100] = 1
	if e % 100 == 0:
		agent.save("./save/BSG-dqn.h5")
		print(collections.Counter(wincounts[int(e/100)-1]))
